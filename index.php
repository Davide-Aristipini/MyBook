<?php 
	...
	if(!isset($_GET['ricerca'])){
		$count="SELECT COUNT(*) FROM `libri` WHERE idutente = ".$_SESSION['id'];
		$result = mysqli_query($connect, $count); 
		$count2 = mysqli_fetch_row($result);

		$ceck="SELECT * FROM `libri` WHERE idutente = ".$_SESSION['id'];
		$result = mysqli_query($connect, $ceck); 
		$user = mysqli_fetch_all($result, MYSQLI_ASSOC); 
	} else {
		$ricerca = $_GET['ricerca'];

		$count="SELECT COUNT(*) FROM `libri` WHERE idutente = ".$_SESSION['id']." AND (titolo LIKE '%".$ricerca."%' OR autore LIKE '%".$ricerca."%')" ;
		$result = mysqli_query($connect, $count); 
		$count2 = mysqli_fetch_row($result);

		$ceck="SELECT * FROM `libri` WHERE idutente = ".$_SESSION['id']." AND (titolo LIKE '%".$ricerca."%' OR autore LIKE '%".$ricerca."%')";
		$result = mysqli_query($connect, $ceck); 
		$user = mysqli_fetch_all($result, MYSQLI_ASSOC); 
        ...
        <div class="content-body">
			...
			<div class="container-fluid">
				<div class="row">
					...
					if($count2[0]==0){
						if($controllo==false){
							...
							<div style='text-align: center' class='col-xl-12'> 
								<h1 style='text-transform: uppercase;'>Insertisci il tuo primo libro</h1>
							</div>";
						} else {
							echo "
							<div style='text-align: center' class='col-xl-12'> 
								<h1 style='text-transform: uppercase;'>nessun Risultato per \"$ricerca\"</h1>
							</div>";
						}
					} else if($count2[0]>3) {

						$libri=$count2[0];
						$approssimato=intval($libri/4);

						for($i=0;$i<$approssimato;$i++){
							echo '<div class="col-xl-6 col-xxl-12">
										<div class="row">';
										...
										for($j=0;$j<4;$j++){
											...
											echo	'<div class="col-sm-6">
														<a href="./libro.php?id='.$user[$j + $variabile]['id'].'">
															<div class="card avtivity-card">
																<div class="card-body">
																	<div class="media align-items-center">
																		<span class="activity-icon mr-md-4 mr-3">
																			<img width="100%" src="'.$user[$j + $variabile]['copertina'].'">
																		</span>
																		<div class="media-body">
																			<p class="fs-14 mb-2">'.$user[$j + $variabile]['autore'].'</p>
																			<span class="title text-black font-w600">'.$user[$j + $variabile]['titolo'].'</span>
																		</div>
																	</div>
																	<div class="progress" style="height:5px;">
																		<div class="progress-bar '.$colore.'" style="width: '.round(($user[$j + $variabile]['segnalibro']/$user[$j + $variabile]['pagine'])*100, 1).'%; height:5px;" role="progressbar">
																			<span class="sr-only">Completato al '.round(($user[$j + $variabile]['segnalibro']/$user[$j + $variabile]['pagine'])*100, 1).'%</span>
																		</div>
																	</div>
																</div>
																<div class="effect '.$colore.'"></div>
															</div>
														</a>
													</div>';
												}
						...
						if(($libri % 4) != 0) {
							...
							echo '<div class="col-xl-6 col-xxl-12">
							 ...
							 for($i = $variabile; $i < $variabile + ($libri % 4); $i++) {
								 if($i - $variabile == 0){
									 $colore = "bg-success";
								 } else if($i - $variabile == 1){
									$colore = "bg-secondary";
								 } else if($i - $variabile == 2){
									$colore = "bg-danger";
								 } else {
									$colore = "bg-warning";
								 }
								echo '		<div class="col-sm-6">
												<a href="./libro.php?id='.$user[$i]['id'].'">
														<div class="card avtivity-card">
															<div class="card-body">
																<div class="media align-items-center">
																	<span class="activity-icon mr-md-4 mr-3">
																		<img width="100%" src="'.$user[$i]['copertina'].'">
																	</span>
																	<div class="media-body">
																		<p class="fs-14 mb-2">'.$user[$i]['autore'].'</p>
																		<span class="title text-black font-w600">'.$user[$i]['titolo'].'</span>
																	</div>
																</div>
																<div class="progress" style="height:5px;">
																	<div class="progress-bar '.$colore.'" style="width: '.round(($user[$i]['segnalibro']/$user[$i]['pagine'])*100, 1).'%; height:5px;" role="progressbar">
																		<span class="sr-only">Completato al '.round(($user[$i]['segnalibro']/$user[$i]['pagine'])*100, 1).'%</span>
																	</div>
																</div>
															</div>
															<div class="effect '.$colore.'"></div>
					...
					<div style='text-align: center' class='col-xl-12'>
							<br>
							<a type='button' class='btn btn-rounded btn-primary' href='./inserisci.php'>AGGIUNGI UN LIBRO</a>
					</div>
											...
											<div class="items">
												<div class="card text-center">
													<div class="card-body">
														...
														<h5 class="fs-16 font-w500 mb-1"><a href="app-profile.php" class="text-black">Roberto Carloz</a></h5>
														...
														<div class="d-flex align-items-center justify-content-center">
															...
															<a href="app-profile.php" class="btn-link fs-14">Send Request</a>
														</div>
													</div>
												</div>
											</div>
											<div class="items">
												<div class="card text-center">
													<div class="card-body">
														...
														<h5 class="fs-16 font-w500 mb-1"><a href="app-profile.php" class="text-black">Cindy Moss</a></h5>
											...
											<div class="items">
												<div class="card text-center">
													<div class="card-body">
														...
														<h5 class="fs-16 font-w500 mb-1"><a href="app-profile.php" class="text-black">Ivankov Smurz</a></h5>
											...
											<div class="items">
												<div class="card text-center">
													<div class="card-body">
														...
														<h5 class="fs-16 font-w500 mb-1"><a href="app-profile.php" class="text-black">Louis Simatupang</a></h5>
														<p class="fs-14">Body Building Trainer</p>
														<div class="d-flex align-items-center justify-content-center">
															<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
																<path d="M5.09569 20C4.80437 19.9988 4.51677 19.9344 4.25273 19.8113C3.98868 19.6881 3.75447 19.5091 3.56624 19.2866C3.37801 19.0641 3.24024 18.8034 3.16243 18.5225C3.08462 18.2415 3.06862 17.9471 3.11554 17.6593L3.88905 12.8902L0.569441 9.45986C0.312024 9.19466 0.132451 8.86374 0.0503661 8.50328C-0.0317185 8.14282 -0.0131526 7.76671 0.104033 7.4161C0.221219 7.06549 0.43251 6.75388 0.714792 6.51537C0.997074 6.27685 1.33947 6.12062 1.70453 6.06376L6.20048 5.37325L8.18158 1.13817C8.34755 0.796915 8.60606 0.509234 8.92762 0.307978C9.24917 0.106721 9.6208 0 10.0001 0C10.3793 0 10.751 0.106721 11.0725 0.307978C11.3941 0.509234 11.6526 0.796915 11.8186 1.13817L13.7931 5.36719L18.2955 6.06376C18.6606 6.12062 19.003 6.27685 19.2852 6.51537C19.5675 6.75388 19.7788 7.06549 19.896 7.4161C20.0132 7.76671 20.0318 8.14282 19.9497 8.50328C19.8676 8.86374 19.688 9.19466 19.4306 9.45986L16.1144 12.8765L16.885 17.66C16.9463 18.0327 16.9014 18.4152 16.7556 18.7635C16.6097 19.1119 16.3687 19.4121 16.0602 19.6297C15.7517 19.8473 15.3882 19.9735 15.0113 19.994C14.6344 20.0144 14.2593 19.9281 13.9292 19.7451L10.0026 17.5635L6.07117 19.7451C5.77302 19.9118 5.43724 19.9996 5.09569 20Z" fill="#FFAA29"/>
